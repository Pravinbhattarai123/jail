...
export async function DELETE(_req: Request, context: any) {
    ...
    // Check if any products are associated through subcategories
    const productCount = await prisma.product.count({ where: { subcategory: { categoryId: id } } })
    if (productCount > 0) {
        return NextResponse.json(
            { error: `Cannot delete category: ${productCount} product(s) associated via its subcategories. Delete those products first.` },
            { status: 400 }
        )
    }
    ...
    try {
        await prisma.$transaction(async (tx) => {
            // Delete subcategories under this category first
