...
export async function DELETE(_req: Request, context: any) {
    ...
    const sub = await prisma.subcategory.findUnique({ where: { id }, select: { id: true } })
    if (!sub) return NextResponse.json({ error: 'Subcategory not found' }, { status: 404 })

    // Block delete if any products are associated
    const count = await prisma.product.count({ where: { subcategoryId: id } })
    if (count > 0) {
        return NextResponse.json(
            { error: `Cannot delete subcategory: ${count} product(s) associated. Delete them first.` },
            { status: 400 }
        )
    }
